...
public abstract class GameObj extends Component {
	...
	public Tile intersectsTileMap(TileMap map) {
		if (map == null) {
			throw new NullPointerException();
		}
		for (Tile t : map.getCurrTiles()) {
			if (intersects(t)) {
				return t;
			}
		}
	...
	public List<Mob> intersectsMobs(MobSpawner mobSpawner) {
		if (mobSpawner == null) {
			throw new NullPointerException();
		}
		List<Mob> accum = new LinkedList<Mob>();
		for (Mob m : mobSpawner.getMobs()) {
			if (intersects(m)) {
				accum.add(m);
			}
		}
	...
	public Coin intersectsCoin(TileMap map) {
		for (Coin c : map.getCurrCoins()) {
			if (intersects(c)) {
				return c;
			}
		}
	...
	/**
	 * Update the velocity of the object in response to hitting an obstacle to
	 * 0. If the direction is null, this method has no effect on the object.
	 */
	public void bounceReduce(Direction d) {
		if (d == null)
			return;
		switch (d) {
		case UP:
	...
	/**
	 * Subclass needs to override this method based on how their object should
	 * appear.
	 * 
	 * @param g
	 */
	...
	/**
	 * Subclass needs to override this method based on how their object should
	 * change whenever the game is updated
	 */
